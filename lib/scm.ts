...
import { normalizeLeadtimeGap, normalizeStockoutKpi, normalizeStockoutRisk, type DemandProfile, type ForecastModel, type ForecastRun, type ForecastSettings, type LeadtimeGap, type LeadtimePolicy, type ModelComparisonPoint, type ModelPerformance, type StockoutKpi, type StockoutRisk, type InventoryProjection } from './scm-model';

export async function getInventoryProjection(): Promise<{ rows: InventoryProjection[]; error: string | null }> {
  try { const supabase = await createSupabaseServerClient(); const { data, error } = await supabase.schema('analytics').from('v_inventory_projection').select('*').order('item_id').order('period'); if (error) return { rows: [], error: error.message }; return { rows: (data ?? []).map((r) => ({ itemId: String(r.item_id), itemName: String(r.item_name ?? r.item_id), period: String(r.period), beginningInventory: r.beginning_inventory == null ? null : Number(r.beginning_inventory), scheduledReceipt: r.scheduled_receipt == null ? null : Number(r.scheduled_receipt), confirmedSalesOrder: r.confirmed_sales_order == null ? null : Number(r.confirmed_sales_order), softAllocation: r.soft_allocation == null ? null : Number(r.soft_allocation), forecastDemand: r.forecast_demand == null ? null : Number(r.forecast_demand), endingProjectedInventory: r.ending_projected_inventory == null ? null : Number(r.ending_projected_inventory), reasonCode: r.reason_code as string | null })), error: null }; } catch (e) { return { rows: [], error: e instanceof Error ? e.message : 'Inventory Projection 조회에 실패했습니다.' }; }
}

export async function getLeadtimePolicies(): Promise<{ rows: LeadtimePolicy[]; error: string | null }> {
  try { const supabase = await createSupabaseServerClient(); const { data, error } = await supabase.schema('analytics').from('v_leadtime_policy').select('*').order('supplier_id'); if (error) return { rows: [], error: error.message }; return { rows: (data ?? []).map((r) => ({ supplierId: String(r.supplier_id), supplierName: String(r.supplier_name ?? r.supplier_id), country: String(r.country ?? '—'), sampleCount: Number(r.n_samples ?? 0), p50: r.p50_days == null ? null : Number(r.p50_days), p80: r.p80_days == null ? null : Number(r.p80_days), p90: r.p90_days == null ? null : Number(r.p90_days), confirmedLeadTime: r.confirmed_lead_time == null ? null : Number(r.confirmed_lead_time), effectiveLeadTime: r.effective_lead_time == null ? null : Number(r.effective_lead_time), source: String(r.source ?? '—'), confirmedAt: r.confirmed_at as string | null })), error: null }; } catch (e) { return { rows: [], error: e instanceof Error ? e.message : 'Lead Time 조회에 실패했습니다.' }; }
}
